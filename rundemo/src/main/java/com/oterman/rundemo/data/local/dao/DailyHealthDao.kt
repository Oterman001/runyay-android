...
interface DailyHealthDao {
     ...
     */
    @Query("SELECT * FROM daily_health WHERE userId = :userId AND vo2Max IS NOT NULL AND vo2Max > 0 ORDER BY calendarDate DESC LIMIT 1")
    suspend fun getLatestVo2Max(userId: String): DailyHealthEntity?

    /**
     * 获取最新两条VO2Max记录（用于delta计算：current - previous）
     */
    @Query("SELECT * FROM daily_health WHERE userId = :userId AND vo2Max IS NOT NULL AND vo2Max > 0 ORDER BY calendarDate DESC LIMIT 2")
    suspend fun getLatestTwoVo2Max(userId: String): List<DailyHealthEntity>

    /**
     * 获取指定日期的最大VO2Max（跨平台取最大值）
     */
    @Query("SELECT MAX(vo2Max) FROM daily_health WHERE userId = :userId AND calendarDate = :calendarDate AND vo2Max IS NOT NULL AND vo2Max > 0")
    suspend fun getVo2MaxForDate(userId: String, calendarDate: String): Double?

    /**
     * 获取指定日期之前最近的VO2Max记录（用于delta计算）
     */
    @Query("SELECT vo2Max FROM daily_health WHERE userId = :userId AND calendarDate < :beforeDate AND vo2Max IS NOT NULL AND vo2Max > 0 ORDER BY calendarDate DESC LIMIT 1")
    suspend fun getPreviousVo2Max(userId: String, beforeDate: String): Double?

    /**
     * 获取指定日期、指定平台的VO2Max（同平台精确匹配）
     */
    @Query("SELECT vo2Max FROM daily_health WHERE userId = :userId AND platformCode = :platformCode AND calendarDate = :calendarDate AND vo2Max IS NOT NULL AND vo2Max > 0 ORDER BY calendarDate DESC LIMIT 1")
     ...
     */
    @Query("SELECT vo2Max FROM daily_health WHERE userId = :userId AND platformCode = :platformCode AND calendarDate < :beforeDate AND vo2Max IS NOT NULL AND vo2Max > 0 ORDER BY calendarDate DESC LIMIT 1")
